...
Player::Player(const std::vector<int>& laneYs) 
    : laneYs_(laneYs), currentLane_(1) {
    shape_.setSize(sf::Vector2f(PLAYER_SIZE, PLAYER_SIZE));
    shape_.setFillColor(PLAYER_COLOR);
    shape_.setPosition(WIDTH / 2 - PLAYER_SIZE / 2, laneYs_[currentLane_] + THREADMILL_HEIGHT + PLAYER_OFFSET_Y);
}
...
void Player::switchLane(int direction) {
    ...
    if (newLane >= MIN_LANE && newLane <= MAX_LANE) {
        ...
        shape_.setPosition(shape_.getPosition().x, laneYs_[currentLane_] + THREADMILL_HEIGHT + PLAYER_OFFSET_Y);
    }
}
